...
------------------------------------------
Each line is one complete sample:

    <sentence1>. <sentence2>. ... <sentenceN>. Why did <subject> <verb>?

The last sentence (starting with "Why") is treated as the question.
Everything before it is the narrative / context.
...
Usage:
    ...
    python run_eval_pg_gpt2.py --use_gold --verbose
"""
...
def load_tellmewhy_txt(path: str, num_samples: int, use_gold: bool) -> list[dict]:
    ...
    The file shown in the task has the format:
        "Jay took a trip ... Why did He go ?"

    We split on the LAST occurrence of "Why" to separate narrative from
    the why-question, then use the full line as the prompt Q.
    If a second column (answer) exists after a tab it is used as gold_answer.
    ...
    with open(path, "r", encoding="utf-8") as fh:
        for raw_line in fh:
            ...
            # Check for tab-separated answer column
            parts = line.split("\t")
            full_text  = parts[0].strip()
            gold_ans   = parts[1].strip() if (use_gold and len(parts) > 1) else None

            # The full_text IS the prompt (narrative + why-question)
            # as seen in the dataset: "... He had a good time. Why did He go ?"
            if not full_text:
                continue
